  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 w-full z-50 flex items-center justify-between px-6 py-5 md:px-12 md:py-6 transition-all duration-700 ease-in-out ${show ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'} ${isMenuOpen ? 'bg-black/90 backdrop-blur-md' : 'bg-transparent'}`}>
        {/* Logo */}
        <a href="#home" className="text-white font-black text-lg md:text-xl tracking-widest uppercase cursor-pointer relative z-50 hover:text-[#ccff00] transition-colors">
          DHARANI PRASANNA
        </a>
        ...
        <div className="hidden md:flex items-center gap-6 lg:gap-8">
          {navLinks.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-gray-300 text-xs lg:text-sm hover:text-[#ccff00] transition-colors uppercase tracking-wider font-medium"
            >
              {item.name}
            </a>
          ))}
        ...
        <div className="flex flex-col items-center gap-6">
          {navLinks.map((item, index) => (
            <a
              key={item.name}
              href={item.href}
              className={`text-white text-2xl font-black uppercase tracking-widest hover:text-[#ccff00] transition-all duration-500 delay-${index * 100} ${isMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
